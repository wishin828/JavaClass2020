...
public class EditProductFiler implements Filter {
        ...
        if (product == null) {
            ...
            if (req.getMethod().equalsIgnoreCase("POST")) {
                ...
                try {
                    ...
                    pm.update(product);
                    pm.setStatus("Product Updated sucessfully");
                } catch (NumberFormatException ex) {
                    pm.setErrors(true);
                    pm.setStatus("Price '" + pprice + "' is not a valid number");
                } catch (DateTimeParseException ex) {
                    pm.setErrors(true);
                    pm.setStatus("Best before date '" + pdate + "' format should be: yyyy-MM-dd");
                } catch (Exception ex) {
